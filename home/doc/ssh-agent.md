...
- Only the intended key is offered: `ssh -v -T git@github.com 2>&1 | grep 'Offering'`
  — one line, not thirteen

## Which key gets offered

`IdentitiesOnly no` is the default, and it means ssh offers **every key the agent
holds** before it touches any `IdentityFile`. Measured against a throwaway sshd
with the stock `MaxAuthTries 6`: an 8-key agent queued 14 candidates, got 6
offers in, and was disconnected with *"Too many authentication failures"* having
never reached the authorised key. The 1Password agent here serves **thirteen**.

So `github.com` and the Teleport hosts pin `IdentitiesOnly yes`. The pin names a
**public** key — ssh matches it against the agent and lets the *agent* sign, so
no private key needs to be on disk. Verified live: one offer, accepted,
`explicit agent`.

Two traps worth knowing:

- **`IdentityFile` is cumulative; `IdentityAgent` and `IdentitiesOnly` are
  first-wins.** A per-host `IdentityFile` does not displace the one in `Host *`
  — both are offered.
- 🔴 **The syketech pin lives ABOVE the "generated by tsh" section, not inside
  it.** `tsh config` rewrites that block wholesale (it is already why the
  absolute-path `ProxyCommand` keeps returning as drift), so anything placed
  inside is silently reverted on the next Teleport touch. First-wins is what
  makes being above sufficient. The harness asserts the ordering.
