...
print_status() {
    ...
    echo ""
    echo "Agent priority (INTERACTIVE shells; env.sh decides for every other shell):"
    echo "  1. An inherited, live SSH_AUTH_SOCK (ssh -A, a multiplexer pane) —"
    echo "     probed with ssh-add -l and kept if it answers. 1Password is a"
    echo "     DEFAULT for a shell that arrived with nothing, never an override."
    echo "  2. 1Password agent (~/.1password/agent.sock + op CLI) — if present"
    echo "     (sets TELEPORT_USE_LOCAL_SSH_AGENT=false automatically)"
    echo "  3. Systemd OpenSSH socket (our ssh-agent.socket)"
    echo "  4. keychain fallback"
    echo "  5. Manual ssh-agent"
    echo ""
    echo "Run ./doctor (or dotfiles-doctor) for the 'agent' row: which socket this"
    echo "shell actually selected, its origin, and whether anything answers on it."
    echo ""
