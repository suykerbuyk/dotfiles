...
if command -v ssh >/dev/null 2>&1 && [[ -r "$SB/.ssh/config" ]]; then
    ...
    # IdentitiesOnly stops the agent flooding a server with every key it holds.
    # Measured against a throwaway sshd with the stock MaxAuthTries 6: an 8-key
    # agent queued 14 candidates, got 6 offers in, and was disconnected having
    # never reached the authorised key. This agent serves THIRTEEN.
    _io_gh=$(ssh -F "$SB/.ssh/config" -G github.com 2>/dev/null | grep -i '^identitiesonly ' | sed 's/^[Ii]dentities[Oo]nly //')
    assert_eq "ssh -G: github.com pins IdentitiesOnly"   "$_io_gh"   "yes"
    _io_sy=$(ssh -F "$SB/.ssh/config" -G hal9k-rpi04.syketech.com 2>/dev/null | grep -i '^identitiesonly ' | sed 's/^[Ii]dentities[Oo]nly //')
    assert_eq "ssh -G: the Teleport hosts pin IdentitiesOnly" "$_io_sy" "yes"
    # The pin names a PUBLIC key on purpose: ssh matches it against the agent and
    # lets the AGENT sign, so no private key need be on disk. Verified live:
    # "Server accepts key: .../id_ed25519.pub ... explicit agent".
    _if_gh=$(ssh -F "$SB/.ssh/config" -G github.com 2>/dev/null | grep -ci '^identityfile .*id_ed25519\.pub' | tr -d ' ')
    assert_eq "ssh -G: github.com offers a .pub, not a private key" "$_if_gh" "1"
    unset _g_gh _g_syke _fa_v _fa_gh _fa_syke _io_gh _io_sy _if_gh
else
...
# 🔴 The syketech IdentitiesOnly pin must sit ABOVE the "generated by tsh"
# section. `tsh config` rewrites that block wholesale — it is already why the
# absolute-path ProxyCommand keeps reappearing as drift — so a setting placed
# inside it is silently reverted on the next Teleport touch. IdentitiesOnly is
...
assert "ssh config: both tsh-ordering anchors were found" \
    "[[ -n \"\$_ln_pin\" && -n \"\$_ln_gen\" ]]"
assert "ssh config: the syketech pin precedes the tsh-generated block" \
    "[[ -n \"\$_ln_pin\" && -n \"\$_ln_gen\" && \$_ln_pin -lt \$_ln_gen ]]"
unset _ln_pin _ln_gen
